"""
Daily Cargo Bot — Vercel Serverless
Handles Slack Events API webhooks (HTTP POST).
Processing is synchronous; Slack retries are caught by dedup on slack_ts.
"""
import json
import os
import re
...
def fetch_url_content(urls, max_chars=6000):
    ...
    for url in urls[:3]:
        try:
            resp = requests.get(url, headers={'User-Agent': ua, 'Accept-Language': 'ja,en;q=0.9'},
                                timeout=12)
            ...
            if content:
                ...
                print(f'URL fetched {url[:60]} → {len(content)} chars')
...
def analyze_with_gemini(text=None, image_bytes=None, mime=None):
    ...
    for model_name, wait in schedule:
        try:
            ...
            err = str(e).lower()
            is_overload = any(k in err for k in ('503', '429', 'overloaded', 'resource_exhausted', 'unavailable'))
            print(f'Gemini error ({model_name}): {str(e)[:100]}')
            if wait and (is_overload or '404' not in err):
                time.sleep(wait)
    ...
    def do_POST(self):
        try:
            ...
            if body.get('type') == 'url_verification':
                response = json.dumps({'challenge': body['challenge']}).encode()
