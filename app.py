...
CATEGORY_EMOJI_ZH = {'运输异常': '🚨', '价格・舱位': '💰', '重点行业需求': '🏭', '政策・通关': '📜', '市场动向・实绩': '📊'}
KEYCAPS = ['①','②','③','④','⑤','⑥','⑦','⑧','⑨','⑩',
           '⑪','⑫','⑬','⑭','⑮','⑯','⑰','⑱','⑲','⑳']
...
- 国际航空货物运输（进出口）
- 国际随身携带（成田/羽田/中部/关西/福冈/新千岁 6大机场对应）
- 日中间高速渡轮海运
- 24小时365天自营报关业务
- 冷链包车（含温控车）
- CellChain LOGISTICS（锂电池等危险品运输）
- 主要客户需求：AOG（航空器部件）、半导体、医药品、汽车零部件、展览会、温控货物、DG（危险品）

→ 关注重点：中日航线、航空舱位、价格、通关、危险品/温控

# 【分类定义】
1. **运输异常**: 延误、取消、混乱、舱位不足、罢工、天气、机场关闭
2. **价格・舱位**: 航空运费、海运运费、现货价格、附加费、燃油附加费、运价调整
3. **重点行业需求**: AOG、医药品、半导体、汽车零部件、展览会、锂电池、温控、危险品、生鲜品、食品出口
4. **政策・通关**: 通关手续、法规修订、关税、贸易协定、进出口管制、危险品规则、机场运营政策
5. **市场动向・实绩**: 机场货物吞吐量、航空公司实绩、市场份额、航线供需动向、行业统计

# 【规则】
- 只选最符合的1个分类
- 所有字符串字段用**中文**输出
- key_data: 本文中具体数字・日期・金额（没有则空数组）
- routes: 具体机场代码・港口名・航线（没有则空数组）
- impact: 从公司视角说明对紧急货物业务的影响（60字以内）
- 不确定时 match: false

# 【输出格式（命中）】
{"match":true,"category":"运输异常","headline":"30字以内","summary":"120字以内","key_data":[],"routes":[],"keywords":[],"excerpt":"最重要的1句","impact":"60字以内","article_title":"","ocr_text":"提取全文"}

# 【输出格式（未命中）】
{"match":false,"reason":"15字以内","ocr_text":"提取全文"}

# 【输出要求】仅输出JSON。禁止markdown・说明文。
---
判断对象:"""
...
3. **重点業界ニーズ**: AOG、医薬品、半導体、自動車部品、展示会、リチウム電池、温度管理、危険品、生鮮品・食品輸出、水産物・青果物、チャーター需要
4. **政策・通関**: 通関手続き、規制改正、関税、貿易協定、輸出入規制、危険品規則、空港オペレーション政策、空港DX・デジタル化、グランドハンドリング政策・規制
...
def save_raw(ev):
    row = {
        ...
        'language':    detect_language(ev.get('text', '')),
    }
    resp = http.post(
            ...
            if a.get('key_data'):
                def fmt_item(x):
                    if isinstance(x, dict):
                        return f'{x.get("value", "")}{x.get("unit", "")}'
                    return str(x)
                detail.append(f'📌 数値: {" / ".join(fmt_item(x) for x in a["key_data"])}')
            if a.get('routes'):
                def fmt_route(x):
                    if isinstance(x, dict):
                        return str(x.get('name', x.get('value', x)))
                    return str(x)
                detail.append(f'✈️ 航路: {" / ".join(fmt_route(x) for x in a["routes"])}')
            if a.get('impact'):
...
def format_daily_report_zh(matches):
    date_str = datetime.now(ZoneInfo('Asia/Tokyo')).strftime('%Y年%m月%d日')
    lines = [f'*📋 中国货运日报 — {date_str}*', '']
    grouped = {}
    for entry in matches:
        cat = (entry.get('analysis') or {}).get('category', '其他')
        grouped.setdefault(cat, []).append(entry)
    seq = 1
    for cat in CATEGORY_ORDER_ZH:
        items = grouped.get(cat)
        if not items:
            continue
        emoji = CATEGORY_EMOJI_ZH.get(cat, '📊')
            ...
            if a.get('key_data'):
                def fmt_item_zh(x):
                    if isinstance(x, dict):
                        return f'{x.get("value", "")}{x.get("unit", "")}'
                    return str(x)
                detail.append(f'📌 数据: {" / ".join(fmt_item_zh(x) for x in a["key_data"])}')
            if a.get('routes'):
                def fmt_route_zh(x):
                    if isinstance(x, dict):
                        return str(x.get('name', x.get('value', x)))
                    return str(x)
                detail.append(f'✈️ 航线: {" / ".join(fmt_route_zh(x) for x in a["routes"])}')
            if a.get('impact'):
                detail.append(f'🎯 业务影响: {a["impact"]}')
            if detail:
                lines.append(' ｜ '.join(detail))
            if urls:
                lines.append(f'🔗 <{urls[0]}|原文链接>')
            lines.append('')
            seq += 1
    return '\n'.join(lines)
...
def process_event(ev, row_id):
    ...
    if text.strip() in REPORT_TRIGGERS:
        ...
        try:
            ...
            if not ja_matches and not zh_matches:
                ...
                if ja_matches:
                    slack_post(OUTPUT, format_daily_report(ja_matches))
                if zh_matches:
                    slack_post(OUTPUT, format_daily_report_zh(zh_matches))
                parts = []
                if ja_matches:
                    parts.append(f'日本語{len(ja_matches)}件')
                if zh_matches:
                    parts.append(f'中文{len(zh_matches)}件')
                slack_post(channel, f'✅ 日報送信完了（{"・".join(parts)}）', thread_ts=ts)
        except Exception as e:
    ...
    try:
        if all_files:
            ...
            result = analyze_with_gemini(image_bytes=file_bytes, mime=mime, prompt=prompt)
        else:
            sb_patch(row_id, {'ocr_text': fetched_text})
            result = analyze_with_gemini(text=analysis_text, prompt=prompt)
    except Exception as e:
